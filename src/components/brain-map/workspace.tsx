  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        <p className="text-[11px] font-medium uppercase tracking-wide text-muted">지금 학습 중</p>
        <p className="mt-0.5 text-lg font-bold break-keep">{node.label}</p>
        <p className="mt-0.5 text-xs text-muted">{node.eyebrow}</p>
      </div>
      {step ? <span className="shrink-0 text-xs tabular-nums text-muted">{step}</span> : null}
    </div>
  )
}
    ...
    <section className="rounded-xl border border-line bg-surface px-5 py-5">
      ...
      {answered ? (
        <div className="mt-5 animate-rise border-t border-line pt-4">
          ...
          {exercise.concept ? (
            <p className="mt-3 border-l-2 border-brand/30 pl-3 text-sm leading-relaxed text-muted break-keep">
              {exercise.concept}
            </p>
          ) : null}
    ...
    <>
      ...
      {!revealed ? (
        ...
        <div className="mt-4">
          ...
          <p className="mt-3 mb-2 text-xs text-muted">내 해석과 비교했을 때 어땠나요?</p>
