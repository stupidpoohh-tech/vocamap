...
export default async function StudentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="animate-rise">
      <Link href="/teacher" className="text-sm text-ink-3 hover:text-ink">
        ← 교사
      </Link>
      ...
      <div className="mb-8 grid grid-cols-3 gap-3">
        <Card className="p-4">
          <p className="text-xs text-ink-3">학습 중 카드</p>
          <p className="mt-1 text-2xl font-semibold tabular-nums">{summary.cards}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-ink-3">총 복습</p>
          <p className="mt-1 text-2xl font-semibold tabular-nums">{summary.reps}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-ink-3">망각 횟수</p>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold">헷갈리는 단어 짝</h2>
        {confusions.length === 0 ? (
          <EmptyState
            title="아직 혼동 기록이 없어요"
            ...
            {confusions.map((pair) => (
              <li key={pair.pairId} className="card flex items-center justify-between px-5 py-3.5">
                <span className="font-semibold">
                  {pair.lemmaA} <span className="text-ink-3">↔</span> {pair.lemmaB}
                </span>
